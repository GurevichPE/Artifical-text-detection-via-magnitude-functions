...
def main():
    ...
    EMBEDDINGS_PATH = "/workspace/mnt/local/data/pgurevich/magnitude"
    ...
    for n in tqdm(MAX_TOKEN_LENGTHS):
        if MODE == 'magnitude':
            with open(f"{MAGNITUDE_PATH}/magnitude_f_{n}_{N_STEPS}_steps.pkl", 'rb') as f:
                mags = pkl.load(f)
            mags = np.array(mags)
        elif MODE == 'embeddings':
            mags = torch.load(f"{EMBEDDINGS_PATH}/embeddings_{n}.pt")
            ...
            with open(f"{MAGNITUDE_PATH}/magnitude_f_{n}_{N_STEPS}_steps.pkl", 'rb') as f:
                mags = pkl.load(f)
            mags = np.array(mags)
            embs = torch.load(f"{EMBEDDINGS_PATH}/embeddings_{n}.pt")
